...
class Commonwidget extends CI_Controller 
{
	...
	public function save_video(){
		...
		if(count($_POST) > 0):
			...
			$Dropbox_link =$this->input->post('upload_link');
			...
			$DescriptionCount=$this->input->post('description_count');
			if($DescriptionCount==0){
				...
				for($i=1;$i<=$DescriptionCount;$i++):
					$MoreDescription['name']=$this->input->post('member_name_'.$i);
					$MoreDescription['role']=$this->input->post('member_role_'.$i);
					$MoreDescription['college']=$this->input->post('member_college_'.$i);
					$MoreDescription['dob']=$this->input->post('member_dob_'.$i);
					$MoreDescription['contact']=$this->input->post('member_contact_'.$i);
					$MemberDetails['details'][]=$MoreDescription;
				endfor;
				$MoreDescription=json_encode($MemberDetails);
			}
			$FormData=['title'=>$Title,'duration'=>$Duration,'dropbox_link'=>$Dropbox_link,'language'=>$Language,'summary'=>$Summary,'year_month_competition'=>$Year_month,'member_location'=>$Location,'teammembers'=>$MoreDescription,'equipment_used'=>$Equipment,'awards'=>$Awards];
			$this->load->database();
			$Result=$this->db->insert('shortfilm_master',$FormData);
			if($Result==1):
				redirect('/shortfilm?process=success&user_upload=true');
			else:
				redirect('/shortfilm?process=failure&user_upload=true');
			endif;
			
		else:
			redirect('/adcheck?process=failure');
		endif;
	}
