...

{
...

	{
		...
		if (check_article_id($article_id) != 0)
		{
			...
			$seo_tags 			= $get_seo_detail['tag_id'];
			...
			$data['seo_tags'] 	= $this->common_model->get_tags_by_id($seo_tags);
		...
		if($contentId!=''){
			...
			if($checkId==0){
				...
				if(count($details) > 0){
					switch($type){
						case 'koo' :
							$headers = ['Content-Type:application/json' , 'X-KOO-API-TOKEN:772ef55a-e9e8-42da-b86e-31871874bdfa'];
