...

{
	...
	*/
...

	{
		...
		if(isset($seo_txtTags) != '')
			$seo_txtTags_id 		= PrepareTagInputValue($seo_txtTags);
		else
			$seo_txtTags_id = '';
		...
		$null_value 		= "NULL";
			...
			if($article_id != '' && $article_id != 'NULL') {
				...
				$this->live_db->insert('seo_tags', ['tag_id' => $seo_txtTags_id, 'tag_name' => $seo_txtTags,	'content_id' => $article_id,
						'updated_by' => USERID]);
...

		{
			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				$rows = $this->live_db->query("SELECT tag_name FROM seo_tags WHERE content_id='".$article_id."'")->num_rows();
				if($rows == 1){
					$this->live_db->where('content_id', $article_id);
					$this->live_db->update('seo_tags', ['tag_id' => $seo_txtTags_id, 'tag_name' => $seo_txtTags, 'updated_by' => USERID]);
				}else {
					$this->live_db->insert('seo_tags',['tag_id' => $seo_txtTags_id, 'tag_name' => $seo_txtTags, 'content_id' => $article_id, 'updated_by' => USERID]);
				}
				$article_details['url'] = $article_details['url']."-".$article_id.".html";
	...
	*/
	...
	{
		return $this->live_db->query('SELECT tag_id,tag_name FROM seo_tags WHERE content_id ='.$article_id.'');
	}
